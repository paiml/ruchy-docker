# Multi-stage Dockerfile for Prime Sieve benchmark (Python)
...
COPY benchmarks/primes/main.py .
...
COPY --from=builder /build/main.py /app/primes.py
...
# Set entrypoint
ENTRYPOINT ["python3", "/app/primes.py"]

# Metadata labels
LABEL org.opencontainers.image.title="Prime Sieve Benchmark (Python)"
